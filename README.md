...
- LLM 聊天模組：使用者 @ 機器人時，由 Gemini API 以貓娘語氣回覆，同時保護系統提示與模型資訊。
- 設定面板：於指定頻道使用 `/config_panel` 顯示互動面板，可編輯 FAQ、禁止字詞、重新載入設定並檢視即時日誌。
  - 亦可透過「編輯 LLM 設定」更新模型、人格提示、最大句數與 API Key 池（遮罩顯示）。
  - `/config_set_channel` 可將當前頻道設定為面板限定頻道。
- 配置集中於 `config/settings.json` 與 `.env`，Gemini API 支援多金鑰輪替。
...
3. 建立 `.env`（可由 `.env.example` 複製）並填入：
   - `DISCORD_TOKEN`
   - `GEMINI_API_KEY`（主要金鑰）
   - `GEMINI_API_KEYS`（選填，多個金鑰逗號分隔，用於自動切換）
...
4. 編輯 `config/settings.json`：
   ...
   - `llm.api_keys`：可選，提供多組金鑰，會在配額用盡時自動切換
